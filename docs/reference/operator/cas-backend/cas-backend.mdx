...
- **Abstracts away the underlying storage backend**. Currently, we support OCI registries as storage backend but you can expect blob storage, Artifactory and other storage backends to be supported in the future.  
...
The inline backend **embeds** the pieces of evidence in the resulting attestations. This is useful to get started quickly but since the pieces of evidences are embedded in the attestation, their size is limited.
