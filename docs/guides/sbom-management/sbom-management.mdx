...
<Image img={require("/docs/guides/dependency-track/overview.png")} />
...
chainloop workflow contract update \
 --id $CONTRACT_ID \
 --name my-contract \
 -f https://github.com/chainloop-dev/docs/blob/main/examples/contracts/sbom/cyclonedx.yaml
```
...
Although the workflow run process can be complex, we will highlight the three most critical steps here. All necessary commands are detailed in the example GitHub [github.yml](https://github.com/chainloop-dev/docs/blob/main/examples/ci-workflows/github.yaml).
