...
You can also use interpolation to calculate the project name dynamically based on the annotations set in the materials. For example, let's say that you have the following material in the contract
...
You can then use the `{{ .Material.Annotations.component }}` template during attachment, for example
